...
type Pokemon struct {
	...
	tipo string
	hp, atk, def int
	// evolucao Pokemon  // tipos recursivos não são permitidos
	evolucao *Pokemon // apenas com ponteiros
}

func main() {
	// são declarados levando em consideração a ordem dos campos
	pikachu := Pokemon{"Pikachu", "Elétrico", 120, 200, 160, &Pokemon{nome: "Raichu"}}
	...
	charmander := &Pokemon{"Charmander", "Fogo", 100, 140, 120, &Pokemon{nome: "Charmeleon"}}
	fmt.Printf("%T\n", charmander)
	fmt.Println(charmander.nome, "é do tipo", charmander.tipo, "e evolui para", charmander.evolucao.nome)
