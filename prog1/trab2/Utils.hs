...
--
-- >>> mean [1.0, 2.2, -1.3, 4.5]
-- 1.6
mean :: [Double] -> Double
mean xs = (sum xs) / (fromIntegral $ length xs)

-- | Calcula o desvio padrao de uma lista de Doubles
--
-- >>> std [1.0, 2.2, -1.3, 4.5]
-- 2.0940391591371927
std :: [Double] -> Double
std xs = (sqrt . mean) [(x-u)^2 | x <- xs]
  where
    u = mean xs
...
-- "  1,  0,  0\n  0,  1,  0\n  0,  0,  1\n"
showMat :: Show a => [[a]] -> String -- Poderiamos utilizar da funcao printf, mas foi optado criar uma funcao para gerar uma string a fim de manter a pureza
